...
--Public
Script.chaseDistance = 60 --float "Chase Distance"
Script.sightRadius = 10 --float "Sight Radius"
Script.speed = 5 --float "Speed"
Script.maxaccel = 5 --float "Max Speed"
Script.health = 30 --int "Health"
...
Script.sound.footsteps={}
Script.sound.footsteps.step={}
Script.sound.footsteps.step[1] = Sound:Load("Sound/Footsteps/step1.wav")
Script.sound.footsteps.step[2] = Sound:Load("Sound/Footsteps/step2.wav")
Script.sound.footsteps.step[3] = Sound:Load("Sound/Footsteps/step3.wav")
Script.sound.footsteps.step[4] = Sound:Load("Sound/Footsteps/step4.wav")
...
function Script:ChooseTarget()
	...
	for k,entity in pairs(entities) do
		if entity:GetKeyValue("type")=="player" then
			if entity.script.alive==true then
...
function Script:UpdatePhysics()
	...
	if self.target then		
		...
		--If the player is dead, stop attacking them
		if self.target.entity:GetKeyValue("type")=="player" then
			if self.target.entity.script ~= nil then
				if self.target.entity.script.alive==false then
					self.target=nil
					self.currentState=self.state.idle
					self.entity:Stop()
					self.followingTarget=false
		...
		if Time:GetCurrent() - self.movemodechangetime > 1000 then
			...
			if self.movemode==1 then				
				if self.entity:GetVelocity():Length()<self.runthreshold then
					...
					self.movemode=0
					self.animationmanager:SetAnimationSequence(self.sequence.walk,0.04,200)
					return
				end
			else
				if self.entity:GetVelocity():Length()>self.runthreshold then
					self.movemodechangetime=Time:GetCurrent()
					self.movemode=1
					self.animationmanager:SetAnimationSequence(self.sequence.run,0.04,200)
					return
				end				
			end
		end
		...
		if self.animationmanager then
			if self.currentState==self.state.idle then
				...
				elseif Time:GetCurrent() > self.lasttimeidlesoundplayed then
					self.lasttimeidlesoundplayed = Time:GetCurrent() + 8000 + Math:Random(-3000,3000)
			...
			elseif self.currentState==self.state.hurt then
				self.animationmanager:ClearAnimations()
				self.stunned = true
				self.animationmanager:SetAnimationSequence(self.sequence.hurt,.02,200,1,self,self.EndStun)	
			elseif self.currentState==self.state.attack then
				if self.stunned==false and self.attackinprogress==false then
					...
					if Time:GetCurrent() > self.sound.attack.lasttimeplayed then
						self.sound.attack.lasttimeplayed = Time:GetCurrent()+ 800 + Math:Random(-200,200)
						self.entity:EmitSound(self.sound.attack[math.random(0,#self.sound.attack)])
					end
					if self.swingmode==1 then
						self.animationmanager:SetAnimationSequence(self.sequence.attack0,0.03,200,1,self,self.EndAttack)
					else
						self.animationmanager:SetAnimationSequence(self.sequence.attack1,0.025,200,1,self,self.EndAttack)
					end
				end
			end
		end
...
--This function plays footstep sounds in regular intervals as the goblin walks
function Script:UpdateFootsteps() 
	local footstepwalkdelay = 500
...
function Script:TakeDamage(damage)
	if self.health>0 then
		...
		if self.health<=0 then
			...
			self.animationmanager:SetAnimationSequence(self.sequence.die,0.03,200,1,self,self.Death)
...
function Script:ManageIdleSounds()
	if Time:GetCurrent() > self.idlesoundtimer then
		self.idlesoundtimer = Time:GetCurrent() + 8000 + Math:Random(-3000,3000)
		self.entity:EmitSound(self.sound.idle[math.random(0,#self.sound.idle)])
	end
end
...
function Script:AttackEnemy(player)
	...
	local entitypos = player:GetPosition(true)
	...
	if pos.z>0 and pos.z<self.attackRange then
		if pos.z>math.abs(pos.x) then
			if pos.z>math.abs(pos.y) then	
				player.script:TakeDamage(10,self.entity)
	...
	if extra~=entity then
		if entity~=nil then
			if entity.script~=nil then
				if entity.script.alive==true then
					extra.script:AttackEnemy(entity)
				end
			end
		end
	end
